...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_PAINT: {
        ...
        HFONT hFont = CreateFont(32, 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE, DEFAULT_CHARSET, 0, 0, 0, 0, L"Segoe UI Symbol");
        ...
        for (int suit = 0; suit < 4; suit++) {
            for (int i = 0; i < 13; i++) {
                ...
                for (int suit = 0; suit < 4; suit++)
                {
                    ...
                    for (int i = 0; i < 13; i++)
                    {
                        // 公式化座標：橫向依點數 i 排開，縱向依花色 suit 動態向下推開（避免 52 張牌重疊）
                        int cardX = 10 + (i * (cardWidth + gap));
                        int cardY = 10 + (suit * (cardHeight + 20)); // 每一列縱向推開「牌高 + 20像素」
                        ...
                        if (i < 10) // A ~ 10 的花色分佈
                        {
                            // --- 【上半部：正立花色】 ---
                            if (i == 0) { // A
                                TextOut(hdc, cx - 10, cy - 15, suits[suit], 1);
                            }
                            else if (i == 1) { // 2
                                TextOut(hdc, cx - 10, cy - dy - 15, suits[suit], 1);
                            }
                            else if (i == 2) { // 3
                                TextOut(hdc, cx - 10, cy - dy - 15, suits[suit], 1);
                                TextOut(hdc, cx - 10, cy - 15, suits[suit], 1); // 中間那顆
                            }
                            else if (i == 3) { // 4
                                TextOut(hdc, cx - dx - 10, cy - dy - 15, suits[suit], 1);
                                TextOut(hdc, cx + dx - 10, cy - dy - 15, suits[suit], 1);
                            }
                            else if (i == 4) { // 5
                                TextOut(hdc, cx - dx - 10, cy - dy - 15, suits[suit], 1);
                                TextOut(hdc, cx + dx - 10, cy - dy - 15, suits[suit], 1);
                                TextOut(hdc, cx - 10, cy - 15, suits[suit], 1); // 正中央
                            }
                            else if (i == 5) { // 6
                                TextOut(hdc, cx - dx - 10, cy - dy - 15, suits[suit], 1);
                                TextOut(hdc, cx + dx - 10, cy - dy - 15, suits[suit], 1);
                                TextOut(hdc, cx - dx - 10, cy - 15, suits[suit], 1); // 中左
                                TextOut(hdc, cx + dx - 10, cy - 15, suits[suit], 1); // 中右
                            }
                            else if (i == 6) { // 7
                                TextOut(hdc, cx - dx - 10, cy - dy - 15, suits[suit], 1);
                                TextOut(hdc, cx + dx - 10, cy - dy - 15, suits[suit], 1);
                                TextOut(hdc, cx - dx - 10, cy - 15, suits[suit], 1);
                                TextOut(hdc, cx + dx - 10, cy - 15, suits[suit], 1);
                                TextOut(hdc, cx - 10, cy - (dy / 2) - 15, suits[suit], 1); // 中間偏上
                            }
                            else if (i == 7) { // 8
                                TextOut(hdc, cx - dx - 10, cy - dy - 15, suits[suit], 1);
                                TextOut(hdc, cx + dx - 10, cy - dy - 15, suits[suit], 1);
                                TextOut(hdc, cx - dx - 10, cy - 15, suits[suit], 1);
                                TextOut(hdc, cx + dx - 10, cy - 15, suits[suit], 1);
                                TextOut(hdc, cx - 10, cy - (dy / 2) - 15, suits[suit], 1); // 中上
                            }
                            else if (i == 8) { // 9
                                TextOut(hdc, cx - dx - 10, cy - dy - 15, suits[suit], 1);
                                TextOut(hdc, cx + dx - 10, cy - dy - 15, suits[suit], 1);
                                TextOut(hdc, cx - dx - 10, cy - (dy / 4) - 15, suits[suit], 1); // 稍微往中間靠
                                TextOut(hdc, cx + dx - 10, cy - (dy / 4) - 15, suits[suit], 1);
                                TextOut(hdc, cx - 10, cy - (dy * 2 / 3) - 15, suits[suit], 1);
                            }
                            else if (i == 9) { // 10
                                TextOut(hdc, cx - dx - 10, cy - dy - 15, suits[suit], 1);
                                TextOut(hdc, cx + dx - 10, cy - dy - 15, suits[suit], 1);
                                TextOut(hdc, cx - dx - 10, cy - (dy / 3) - 15, suits[suit], 1); 
                                TextOut(hdc, cx + dx - 10, cy - (dy / 3) - 15, suits[suit], 1); 
                                TextOut(hdc, cx - 10, cy - (dy * 2 / 3) - 15, suits[suit], 1); // 中間兩顆的上顆
                            }
                        }
                        else // J, Q, K
                        {
                        ...
                        for (int side = 0; side < 2; side++)
                        {
                            if (side == 1) // 倒立設定
                            {
                                ...
                                // 當 side == 1 (倒立) 時，補畫下半部的花色
                                if (i == 1)      TextOut(hdc, cx - 10 + offset, cy - dy - 15, suits[suit], 1); // 2 的下半
                                else if (i == 2) TextOut(hdc, cx - 10 + offset, cy - dy - 15, suits[suit], 1); // 3 的下半
                                else if (i == 3) { // 4 的下半
                                    TextOut(hdc, cx - dx - 10 + offset, cy - dy - 15, suits[suit], 1);
                                    TextOut(hdc, cx + dx - 10 + offset, cy - dy - 15, suits[suit], 1);
                                }
                                else if (i == 4) { // 5 的下半
                                    TextOut(hdc, cx - dx - 10 + offset, cy - dy - 15, suits[suit], 1);
                                    ...
                                    TextOut(hdc, cx - dx - 10 + offset, cy - dy - 15, suits[suit], 1);
                                    TextOut(hdc, cx + dx - 10 + offset, cy - dy - 15, suits[suit], 1);
                                    TextOut(hdc, cx - 10 + offset, cy - (dy / 2) - 15, suits[suit], 1); // 對稱中下
                                }
                                else if (i == 8) { // 9 的下半
                                    TextOut(hdc, cx - dx - 10 + offset, cy - dy - 15, suits[suit], 1);
                                    TextOut(hdc, cx + dx - 10 + offset, cy - dy - 15, suits[suit], 1);
                                    TextOut(hdc, cx - dx - 10 + offset, cy - (dy / 3) - 15, suits[suit], 1);
                                    TextOut(hdc, cx + dx - 10 + offset, cy - (dy / 3) - 15, suits[suit], 1);
                                }
                                else if (i == 9) { // 10 的下半
                                    TextOut(hdc, cx - dx - 10 + offset, cy - dy - 15, suits[suit], 1);
                                    ...
                                    TextOut(hdc, cx - dx - 10 + offset, cy - (dy / 3) - 15, suits[suit], 1);
                                    TextOut(hdc, cx + dx - 10 + offset, cy - (dy / 3) - 15, suits[suit], 1);
                                    TextOut(hdc, cx - 10 + offset, cy - (dy * 2 / 3) - 15, suits[suit], 1);
                                }
                            }

                            // 繪製角標（A, 2, 3... 與對應花色）
                            // 當 side = 0 在左上角正立；當 side = 1 被上面 XFORM 矩陣自動旋轉至右下角倒立
                            TextOut(hdc, cardX + 10, cardY + 5, ranks[i], (int)_tcslen(ranks[i]));
                            TextOut(hdc, cardX + 10, cardY + 30, suits[suit], 1);
